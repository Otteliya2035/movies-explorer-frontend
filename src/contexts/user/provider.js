...
export const UserProvider = ({ children }) => {
  ...
  const fetchUserData = async () => {
    ...
    if(token) {
      try {
        const data = await mainApi.getCurrentUserInfo(token);
        setUserData(data);
        setIsLoggedIn(true);
      } catch (error) {
        setIsLoggedIn(false);
  ...
  const login = async (email, password) => {
    try {
      const { token } = await mainApi.authorize(email, password).catch(() => {
        alert('Неправильный логин или пароль')
      });

      if(token) {
        const user = await mainApi.getCurrentUserInfo(token).then(res => {
          return res
        });
